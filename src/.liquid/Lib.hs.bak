...
import Protolude hiding (bool, die, head, ifM, trace, traceM)
...
makeList n | n > 1 = getRndMInt (1, n) >>= valConsApply -- getRndMInt (1, n) >>= \seed -> (seed :) <$> makeList seed
  where
    valConsApply seed = (seed :) <$> makeList seed
